...
def draw_face(img, eye_open=True):
    ...
    # چشم‌ها
    if eye_open:
        # چشم باز (دایره سفید با مردمک سیاه)
...
# پوشه‌ها رو بساز
os.makedirs("test_faces/open", exist_ok=True)
os.makedirs("test_faces/closed", exist_ok=True)

# ۵۰ تا تصویر با چشم باز
for i in range(50):
    img = np.zeros((240, 200, 3), dtype=np.uint8)
    img = draw_face(img, eye_open=True)
    cv2.imwrite(f"test_faces/open/open_{i}.png", img)

# ۵۰ تا تصویر با چشم بسته
for i in range(50):
    img = np.zeros((240, 200, 3), dtype=np.uint8)
    img = draw_face(img, eye_open=False)
    cv2.imwrite(f"test_faces/closed/closed_{i}.png", img)

print("✅ دیتاست تست با چهره‌های ساده ساخته شد!")
print(f"📁 پوشه: test_faces/")
print(f"🖼️ تعداد تصاویر: ۱۰۰ عدد (۵۰ باز، ۵۰ بسته)")