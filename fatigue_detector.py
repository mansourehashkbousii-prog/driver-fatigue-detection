# ================================================================
# 🚗 DRIVER FATIGUE DETECTION v6.0 - FINAL
# ================================================================
...
class MARDetector:
    ...
    def __init__(self, yawn_threshold=1.0):
        self.mar_value = 0.0
...
class PDFReporter:
    ...
    def generate_report(self, data_logger):
        ...
        try:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"fatigue_report_{timestamp}.pdf"
            c = canvas.Canvas(filename, pagesize=A4)
            width, height = A4
            
            c.setFont("Helvetica-Bold", 24)
            c.drawString(50, height - 50, "Driver Fatigue Detection Report")
            
            c.setFont("Helvetica", 12)
            y = height - 100
            c.drawString(50, y, f"Session: {data_logger.session_id}")
            y -= 30
            c.drawString(50, y, f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            
            if len(data_logger.data['ear']) > 0:
                df = pd.DataFrame(data_logger.data)
                y -= 40
                c.setFont("Helvetica-Bold", 16)
                c.drawString(50, y, "Statistics")
                y -= 30
                c.setFont("Helvetica", 12)
                c.drawString(50, y, f"Samples: {len(df)}")
                y -= 20
                c.drawString(50, y, f"Duration: {len(df)/30:.1f}s")
                y -= 20
                c.drawString(50, y, f"Mean EAR: {df['ear'].mean():.3f}")
                y -= 20
                c.drawString(50, y, f"Mean MAR: {df['mar'].mean():.3f}")
...
class FatigueDashboard:
    def __init__(self, root):
        self.root = root
        self.root.title("🚗 Driver Fatigue Detection v6.0")
    ...
    def _build_gui(self):
        ...
        tk.Label(header, text="🚗 Driver Fatigue Detection v6.0", bg='#0f3460', fg='white', font=('Segoe UI', 18, 'bold')).pack(pady=(10,0))
        ...
        btn_style = {'width': 10, 'height': 1, 'font': ('Segoe UI', 9, 'bold'), 'relief': 'flat', 'cursor': 'hand2'}
        ...
        self.pdf_btn = tk.Button(toolbar, text="📄 PDF", command=self.generate_pdf, bg='#e17055', fg='white', state='disabled', **btn_style)
        self.pdf_btn.pack(side='left', padx=3, pady=12)
    ...
    def generate_pdf(self):
        if len(self.logger.data['ear']) < 10:
            messagebox.showwarning("Warning", "Not enough data!")
            return
        filename = self.pdf.generate_report(self.logger)
        if filename:
            messagebox.showinfo("✅ PDF Generated", f"PDF: {filename}")
        else:
            messagebox.showerror("Error", "PDF failed. Install: pip install reportlab")
    ...
    def update_waveform(self):
        ...
        #    # ===== YAWN + BEEP =====
         #   if self.is_yawning and self.mar_value > 1.2:
  ...
  #              if self.current_state == "AWAKE":
   #                 self.current_state = "DROWSY"
    ...
    def _update_ui(self, ear):
        ...
        if self.threshold.calibrated:
            self.info_labels['cal_val'].config(text="✓", fg='#00b894')
        else:
            self.info_labels['cal_val'].config(text=f"{self.calibration_counter}/150", fg='#fdcb6e')
